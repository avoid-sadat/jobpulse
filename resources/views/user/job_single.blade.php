...
@endif
    ...
    @if(Route::has('login'))
            ...
            <div class="col-6">
                ...
                @else
                    <form action="{{ route('apply', ['job' => $job->id]) }}" method="POST">
                        @csrf
                        <button type="submit" class="btn btn-block btn-primary btn-md text-black">
                            <span class="icon-paper-plane mr-2 text-black"></span> Apply Now
                        </button>
                    </form>
                @endif
            </div>
        @else
            <div class="col-6">
                <button class="btn btn-block btn-primary btn-md"><a href="{{ route('login') }}" class="text-white">You need to register first</a></button>
            </div>
        @endauth
    @endif
</div>
